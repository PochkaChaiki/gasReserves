...
def make_input_data(init_data: pd.DataFrame) -> pd.DataFrame:
    ...
    mid_data['fin_reservoir_pressure'] = np.exp(1293 * 1e-9 * 2700 * float(init_data['relative_density'])) / 1e6 
...
def make_init_data_for_prod_indics(init_data: pd.DataFrame) -> pd.DataFrame:
    mid_data = pd.DataFrame(columns=['lambda_trail', 'lambda_fontain', 'macro_roughness_l', 'filtr_resistance_A', 'filtr_resistance_B'], index=['value'])
    
    if init_data['lambda_trail']['value'] is None:
        mid_data['lambda_trail'] = 0.067 * (2 * init_data['trail_roughness']/ init_data['trail_diameter'])**0.2
    else:
        mid_data['lambda_trail'] = init_data['lambda_trail']['value']
    
    if init_data['lambda_fontain']['value'] is None:
        mid_data['lambda_fontain'] = (2 * np.log10(7.41 * init_data['pipe_diameter']/ 2 / init_data['pipe_roughness']))**(-2)
    else:
        mid_data['lambda_fontain'] = init_data['lambda_fontain']['value']

    if init_data['macro_roughness_l']['value'] is None:
        mid_data['macro_roughness_l'] = 0.425 * 1e-9 * init_data['permeability_k'] ** 1.45 /100
    else:
        mid_data['macro_roughness_l'] = init_data['macro_roughness_l']['value']
    
    if init_data['filtr_resistance_A']['value'] is None:
        mid_data['filtr_resistance_A'] = init_data['viscosity'] *  init_data['init_overcompress_coef']* 0.1013 * init_data['reservoir_temp'] / (np.pi * init_data['permeability_k'] * init_data['effective_thickness'] * 293.15) * np.log(500/0.1) * 11.347 * 1e3
    else:
        mid_data['filtr_resistance_A'] = init_data['filtr_resistance_A']['value']

    if init_data['filtr_resistance_B']['value'] is None:
        mid_data['filtr_resistance_B'] = init_data['density_athmospheric']* 0.1013 *  init_data['init_overcompress_coef'] * init_data['reservoir_temp'] / mid_data['macro_roughness_l'] / (2 * np.pi**2 *  init_data['effective_thickness']**2 * 293.15) * (1/0.1 - 1/500) * 10**(-6) / 86400**2 * 1e3
    else:
        mid_data['filtr_resistance_B'] = init_data['filtr_resistance_B']['value']

    init_data.update(mid_data)
    print(init_data['filtr_resistance_A'], init_data['filtr_resistance_B'])
    return init_data